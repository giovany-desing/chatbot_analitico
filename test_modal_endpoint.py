...
def test_prediction(
    ...
    try:
        ...
        if expected_chart_type and not has_error:
            matches_expected = (
                result.get("chart_type", "").lower() == expected_chart_type.lower()
            )
            validation["matches_expected"] = matches_expected
        
        # Determinar si la prueba fue exitosa
        success = is_valid_json and (matches_expected is None or matches_expected)
        ...
        return {
            ...
            "success": success  # Agregar success en el nivel superior
        }
    ...
    for i, result in enumerate(results, 1):
        ...
        if result.get("success"):
            ...
            if "matches_expected" in validation:
                ...
                expected_type = test_cases[i-1].get("expected_chart_type", "N/A")
                actual_type = result.get("response", {}).get("chart_type", "N/A")
                if match:
                    print(f"   - Tipo de gráfico: {actual_type} ✅ (esperado: {expected_type})")
                else:
                    print(f"   - Tipo de gráfico: {actual_type} ⚠️  (esperado: {expected_type}, pero puede ser válido)")
            else:
                chart_type = result.get("response", {}).get("chart_type", "N/A")
                print(f"   - Tipo de gráfico: {chart_type}")
        else:
            error_msg = result.get('error', 'Unknown error')
            print(f"   - Error: {error_msg}")
            if "response" in result and "error" in result["response"]:
                print(f"   - Detalle: {result['response'].get('error', '')}")
